...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const stopMicrophone = () => {
    try {
      ...
      console.warn("Mic cleanup error:", e);
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resize = () => {
        ...
        glContext.canvas.style.width = w + "px";
        glContext.canvas.style.height = h + "px";
      ...
      let lastTime = 0, currentRot = 0, voiceLevel = 0, isMicInit = false;

      if (enableVoiceControl) {
        initMicrophone().then(ok => { isMicInit = ok; });
      }
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && isMicInit) {
          voiceLevel = analyzeAudio();
          onVoiceDetected?.(voiceLevel > 0.1);
          const speed = 0.3 + voiceLevel * maxRotationSpeed * 2.0;
          if (voiceLevel > 0.05) currentRot += dt * speed;
      ...
      return () => {
        ...
        if (container && glContext?.canvas && container.contains(glContext.canvas)) {
          try { container.removeChild(glContext.canvas); } catch {}
        }
        stopMicrophone();
        glContext?.getExtension("WEBGL_lose_context")?.loseContext();
      };
    } catch (e) {
      console.error("VoicePoweredOrb init error:", e);
      return () => {};
    }
