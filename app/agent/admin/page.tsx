...
//       setStatus('Please enter a valid phone number with country code (e.g., +91...).');
//       return;
//     }

//     setLoading(true);
//     setStatus('Initiating call through telecom network...');

//     try {
//       // Calling the API route we just built!
...
//       if (res.ok) {
//         setStatus(`Success! Ringing ${phoneNumber}... Pick up your phone! 📱`);
//       } else {
//         setStatus(`Error: ${data.error} (Check terminal for details)`);
//         console.error("Dialer Error:", data);
//       }
//     } catch (err) {
...
//   return (
//     <div className="p-8 max-w-md mx-auto mt-20 animate-in fade-in duration-700">
//       <GlassCard className="p-8 space-y-6 flex flex-col items-center border-green-500/30">
...
//           Enter an Indian mobile number to have the Krishi AI Voice Agent call it instantly.
//         </p>
        
//         <div className="w-full space-y-2">
//           <label className="text-xs text-slate-500 uppercase tracking-wider font-bold">Farmer Mobile Number</label>
//           <input 
//             type="tel" 
//             value={phoneNumber}
//             onChange={(e) => setPhoneNumber(e.target.value)}
//             placeholder="+919876543210"
//             className="w-full bg-black/60 border border-white/10 rounded-lg px-4 py-4 text-white focus:border-green-500 focus:outline-none transition-colors text-center text-xl tracking-widest"
//           />
//         </div>

//         <NeonButton onClick={handleCall} disabled={loading} className="w-full py-4 text-lg">
//           {loading ? (
...
//         {status && (
//           <div className={`mt-4 p-4 rounded-lg text-sm w-full text-center border ${status.includes('Error') ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-green-500/10 border-green-500/30 text-green-400'}`}>
//             {status}
//           </div>
//         )}
//       </GlassCard>
...
// import React, { useState } from 'react';
// import { PhoneCall, Loader2, Signal } from 'lucide-react';
...
//       {/* Main Telecom Interface Card */}
//       <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        
//         {/* Header Section (Emerald Branding) */}
...
//               className="w-full px-6 py-5 text-2xl tracking-widest font-mono text-center text-slate-800 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/20 transition-all outline-none shadow-inner"
//             />
//           </div>
...
//   // 🔥 THE FIX: This remembers the last called number so it doesn't loop!
//   const lastDialedNumber = useRef('');

//   const handleCall = useCallback(async (numberToCall: string) => {
//     if (!numberToCall || numberToCall.length < 13) { 
...
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ customerNumber: numberToCall }),
//       });

//       const data = await res.json();

//       if (res.ok) {
//         setStatus(`Success! Ringing ${numberToCall}... Pick up the phone! 📱`);
//       } else {
//         setStatus(`Error: ${data.error || 'Failed to initiate call.'}`);
//         lastDialedNumber.current = ''; // Reset memory if the call failed so you can try again
//       }
//     } catch (err) {
//       setStatus('Network error occurred. Please check your connection.');
//       lastDialedNumber.current = ''; // Reset memory on error
//     }
//     setLoading(false);
//   }, []);

//   // 🔥 THE UPGRADED AUTO-DIALER
//   useEffect(() => {
//     // Check if 13 digits + Not Loading + We haven't already dialed this exact number
//     if (phoneNumber.length === 13 && !loading && lastDialedNumber.current !== phoneNumber) {
//       lastDialedNumber.current = phoneNumber; // Lock it in immediately
//       handleCall(phoneNumber);
//     }
    
//     // If the user starts deleting numbers, reset the memory so they can type a new one
//     if (phoneNumber.length < 13) {
//       lastDialedNumber.current = '';
//     }
//   }, [phoneNumber, handleCall, loading]);
...
//           <button 
//             onClick={() => {
//               lastDialedNumber.current = ''; // Force reset manual call
//               handleCall(phoneNumber);
//             }} 
//             disabled={loading} 
//             className="w-full flex items-center justify-center gap-3 py-5 bg-slate-900 hover:bg-emerald-600 active:bg-emerald-700 text-white text-lg font-bold rounded-2xl shadow-lg hover:shadow-emerald-500/30 transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
